...
import torch
import onnxruntime
from training_exp_mask import Quadrator2D
...
def gen_vel_limit(x, y):
    n = 0.001027
    v0 = 0.2
    v1 = 2*n
    dist = math.sqrt(math.pow(x,2) + math.pow(y,2))
    return round(v0 + v1*dist,4)
    

def simulate_traj(networkName, state, state_exact, docking_radius, theta_rad, max_dist, pert_rad, timeout):
    ...
    u_hover = mass * gravity / 2.0
    u_hover_vec = torch.tensor([u_hover, u_hover], dtype=torch.float32).unsqueeze(0)
    ...
    for i in range(TIMEOUT):
        ...
        if abs(state_exact[0]) < DOCK_RAD and abs(state_exact[1]) < DOCK_RAD and abs(state_exact[2]) < THETA_RAD:
            print("DOCKING SUCCESSFUL")
            dockStatus = True
            break
        if (state_exact[0] >= MAX_DIST and state_exact[3] > 0) or (state_exact[0] <= -MAX_DIST and state_exact[3] < 0) \
            or (state_exact[1] >= MAX_DIST and state_exact[4] > 0) or (state_exact[1] <= -MAX_DIST and state_exact[4] < 0):
        ...
        px0, pz0, th0, vx0, vz0, omega0 = state_exact[0], state_exact[1], state_exact[2], state_exact[3], state_exact[4], state_exact[5]

        forces = torch.tensor(control_exact)
        forces = forces + perturbation

        # delta-around-hover controller
        u = torch.clamp(u_hover_vec + forces, 0.0, 6.0).squeeze(0)
        ...
        ax = -(thrust_sum / mass) * np.sin(th0)
        az = (thrust_sum / mass) * np.cos(th0) - gravity
        alpha = torque / inertia

        # Euler integration
        px_next = px0 + dt * vx0
        pz_next = pz0 + dt * vz0
        theta_next = th0 + dt * omega0
        vx_next = vx0 + dt * ax
        vz_next = vz0 + dt * az
        omega_next = omega0 + dt * alpha


        state_exact = np.array([px_next, pz_next, theta_next, vx_next, vz_next, omega_next], dtype=np.float32)
...
def sample_initial_points_2d_docking(
    ...
    """
    lo = torch.tensor(
        [-env.pos_limit, -env.pos_limit, -env.theta_limit, -env.vel_limit, -env.vel_limit, -env.omega_limit],
    ...
    while remaining > 0 and tries < max_tries:
        ...
        x = torch.rand((M, 6), device=device, dtype=dtype) * (hi - lo) + lo
        px, py = x[:, 0], x[:, 1]
...
if __name__ == "__main__":
    ...
    x0 = sample_initial_points_2d_docking(env, N=numpoints)

    #x0 = torch.Tensor([[-0.35, -1.18, -0.25, 0.0]])

    docking_radius = 0.03
    theta_rad = 0.04 * math.pi
    max_dist = 0.18
    pert_rad = 0.0
    timeout_epochs = 2000
    ...
    for i in range(len(x0)):
        ...
        dockStatus, steps, safe = simulate_traj(networkName_init, x0[i], x0[i], docking_radius, theta_rad,  max_dist, pert_rad, timeout_epochs)
