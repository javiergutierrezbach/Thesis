...
class Queries:
    ...
    def check_descent_safe(self, unsafe_threshold_pos, docking_threshold_pos, input=[[-3, -2], [-3, -2], [-0.5, -0.3], [-0.5, -0.3]]):
        start = datetime.now()
        useMILP = True
    ...
    def run_unroll(self, network):
        ...
        action, init_val, out_val, th_d_1, thdot_d_1 = first_output[0], second_output[0], third_output[0], dyn_output[0], dyn_output[1]

        th_d_0, thdot_d_0, action_d = dyn_input[0], dyn_input[1], dyn_input[2]
        ...
        network.addEquality([th_0, th_d_0], [1, -1], 0, False)
        network.addEquality([thdot_0, thdot_d_0], [1, -1], 0, False)
        network.addEquality([u, action_d], [1, -1], 0, False)
