...
if __name__ == "__main__":
    ...
    initial_controller_file = "/Users/javiergutierrez/Documents/Thesis/Thesis_Repo/Inverted_Pendulum/pend_100_ppo_128.pth"
   ...
   # sample_grid(cur_dynamic_data_file, sample_spacing)
    ...
    ret, ret_ranges, failed = safe_descent_cond_check(cur_comb_file, cur_model_onnx_file, safe_pos = safe_pos, limit_pos = pos_limit, docking_pos = 0.2)
    ...
    while (len(ret) > 0 or is_traj):
        ...
        st_ver_time = datetime.now()
        ret, ret_ranges, failed = safe_descent_cond_check(next_comb_file, next_model_onnx_file,  safe_pos = safe_pos, limit_pos = pos_limit, docking_pos = 0.2)
        end_ver_time = datetime.now()
        diff_ver_time = end_ver_time - st_ver_time
        f = open(out_timing_counterexample_file, "a")
        print("Total verification time for verification index", str(index), ":", str(diff_ver_time.seconds) + "\n")
