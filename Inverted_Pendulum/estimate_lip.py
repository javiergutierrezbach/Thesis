...
def onnx_dyn_controller(dyn_file, controller_file, file_2):
    x = torch.randn(1,3, requires_grad=True, dtype=torch.float32)
    y = torch.randn(1,3, requires_grad=True, dtype=torch.float32)
    z = torch.randn(1,2, requires_grad=True, dtype=torch.float32)
    w = torch.randn(1,2, requires_grad=True, dtype=torch.float32)
    ...
    class OnnxDynController(nn.Module):
        ...
        def forward(self, x, y, z, w):
            outputA = self.dynA(x)
            outputB = self.dynB(y)
            outputC = self.controllerA.net(z)
            outputD = self.controllerB.net(w)
...
def onnx_controller(controller_file, file_2):
    x = torch.randn(1,2, requires_grad=True, dtype=torch.float64)
    y = torch.randn(1,2, requires_grad=True, dtype=torch.float64)
...
def test_lip_network(onnx_path, pivot):
    ...
    th, thdot, u = network.inputVars[0][0]
    th1, thdot1, u1 = network.inputVars[1][0]
    outth, outthdot = network.outputVars[0][0]
    outth1, outthdot1 = network.outputVars[1][0]

    #controller inputs and outputs
    cth, cthdot = network.inputVars[2][0]
    cu = network.outputVars[2][0]
    cth1, cthdot1 = network.inputVars[3][0]
    cu1 = network.outputVars[3][0]

    ranges = [[-0.7,0.7], [-0.7,0.7]]


    # set up bounds of state space
    network.setLowerBound(th, ranges[0][0])
    network.setLowerBound(thdot, ranges[1][0])
    # network.setLowerBound(fx, -1.0)
    # network.setLowerBound(fy, -1.0)

    network.setUpperBound(th, ranges[0][1])
    network.setUpperBound(thdot, ranges[1][1])
    # network.setUpperBound(fx, 1.0)
    # network.setUpperBound(fy, 1.0)

    network.setLowerBound(th1, ranges[0][0])
    network.setLowerBound(thdot1, ranges[1][0])
    # network.setLowerBound(fx1, -1.0)
    # network.setLowerBound(fy1, -1.0)

    network.setUpperBound(th1, ranges[0][1])
    network.setUpperBound(thdot1, ranges[1][1])
    ...
    u_clip = clipForce(network, cu, clip_lb, clip_ub)

    u1_clip = clipForce(network, cu1, clip_lb, clip_ub)

    # equate controller inputs outputs to dynamic 
    network.addEquality([th, cth], [1, -1], 0, False)
    network.addEquality([thdot, cthdot], [1, -1], 0, False)

    network.addEquality([u_clip, u], [1, -1], 0, False)

    network.addEquality([th1, cth1], [1, -1], 0, False)
    network.addEquality([thdot1, cthdot1], [1, -1], 0, False)

    network.addEquality([u1_clip, u1], [1, -1], 0, False)
    ...
    dth, dthdot, du = encodeDifferences(network, [th, thdot, u], [th1, thdot1, u1])
    #dx, dy, dvx, dvy = encodeDifferences(network, [x, y, vx, vy], [x1, y1, vx1, vy1])

    delta = network.getNewVariable()
    network.addMaxConstraint({dth, dthdot}, delta)

    # make sure the points are not equal by adding a lower bound on max norm
    tol = 1e-4
    network.setLowerBound(delta, tol)

    dout_th, dout_thdot = encodeDifferences(network, [outth, outthdot], [outth1, outthdot1])

    epsilon = network.getNewVariable()
    network.addMaxConstraint({dout_th, dout_thdot}, epsilon)
    ...
    lip = binarySearch(query_path, options, delta, epsilon, pivot, 10, None, None)
...
if __name__ == "__main__":
    ...
    dynamic_path = "dynamics/dynamic_0.pt"
    controller_path = "controllers/controller_0.pt"
