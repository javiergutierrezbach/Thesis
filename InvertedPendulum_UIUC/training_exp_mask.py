...
import os
import time
# from training_lip import Trainer_Lip
...
class Controller():
    def __init__(self, file_name="fixed_controller_20n.pt", isInitial=False, t=0.05, device="cpu"):
        ...
        if (isInitial):
            ...
            self.nn.load_state_dict(torch.load(
                file_name, weights_only=False, map_location=self.device))
        else:
            self.nn = torch.load(
                file_name, weights_only=False, map_location=self.device)
    ...
    def next_step(self, states, actions=None):
        ...
        u = 2*torch.clamp(actions, -1, 1).squeeze()
        # print(u)
        newthdot = (1 - b) * thdot + (
...
class Trainer(pl.LightningModule):
    def __init__(self, model, V, controller, datamodule, out_model, out_controller, out_timing, threshold, primal_learning_rate=1e-3, goalfactor=1, decreasefactor=1, nongoalfactor=1, goaleps=1e-4, nongoaleps=1e-4, descenteps=5e-3, safe_level=1, safe_factor=1e2, unsafe_factor=1e2, eps=1e-5):
        super().__init__()
    ...
    # def maximize_in_neighborhood(self, V, x0, epsilon, alpha=0.001, steps=30, norm="linf"):
    #     """
    #     Given input tensor x (n * 4), find the perturbed input in the neighborhood
    #     that maximizes the output V(x).
    #     """
    #     x = x0.clone().detach()
    #     delta = torch.zeros_like(x0, requires_grad=True)  # Perturbation
    #     ori_x = x.clone()
    #     for _ in range(steps):
    #         x.requires_grad = True
    #         V_x = V(x).sum()
    #         V.zero_grad()
    #         V_x.backward()  # Compute gradient
    #         with torch.no_grad():
    #             if norm == "linf":
    #                 # L-infinity: Step in sign direction and project to [-epsilon, epsilon]
    #                 perturb_x = x + alpha * x.grad.sign()
    #                 delta = torch.clamp(perturb_x - ori_x,
    #                                     min=-epsilon, max=epsilon)
    #                 x = torch.clamp(ori_x + delta, min=-0.7, max=0.7).detach_()
    #             elif norm == "l1":
    #                 # L1: Approximate projection using element-wise sorting and scaling
    #                 g = delta.grad  # Gradient
    #                 abs_g = torch.abs(g)
    #                 sorted_g, _ = torch.sort(abs_g, dim=-1, descending=True)

    #                 # Compute cumulative sum and scaling factor
    #                 cumsum_g = torch.cumsum(sorted_g, dim=-1)
    #                 rho = torch.arange(
    #                     1, x0.shape[1] + 1).float()
    #                 scaling = (cumsum_g - epsilon) / rho

    #                 # Find threshold for projection
    #                 threshold = torch.max(
    #                     scaling * (sorted_g > scaling), dim=-1, keepdim=True)[0]
    #                 # Normalized gradient step
    #                 delta += alpha * g / (torch.abs(g) + 1e-8)
    #                 delta *= (abs_g >= threshold)  # Keep largest components
    ...
    # def maximize_in_neighborhood(self, V, x0, epsilon, alpha=0.001, steps=30, norm="linf"):
    #     """
    #     Finds a perturbation of x0 that maximizes V(x), staying within epsilon-ball (L-inf or L1 norm).
    #     Gradient must flow back to x0 (and thus controller).
    #     """
    #     delta = torch.zeros_like(x0, requires_grad=True)  # Leaf tensor with grad

    #     for _ in range(steps):
    #         x = x0 + delta  # This will track grad correctly

    #         V_x = V(x).sum()

    #         grads = torch.autograd.grad(V_x, delta, create_graph=True)[0]
    #         if norm == "linf":
    #             delta = delta + alpha * grads.sign()
    #             delta = torch.clamp(delta, -epsilon, epsilon)
    #         elif norm == "l1":
    #             # L1 projection: same as before, just ensure it's differentiable
    #             g = grads
    #             abs_g = torch.abs(g)
    #             sorted_g, _ = torch.sort(abs_g, dim=-1, descending=True)
    #             cumsum_g = torch.cumsum(sorted_g, dim=-1)
    #             rho = torch.arange(1, x0.shape[1] + 1).float().to(x0.device)
    #             scaling = (cumsum_g - epsilon) / rho
    #             threshold = torch.max(
    #                 scaling * (sorted_g > scaling), dim=-1, keepdim=True
    #             )[0]
    #             delta = delta + alpha * g / (abs_g + 1e-8)
    #             delta = delta * (abs_g >= threshold)
    #             delta = torch.clamp(delta, -epsilon, epsilon)

    #     # Final perturbed x, gradient will flow back to x0
    ...
    def local_lipschitz_loss(self, x, local_lip_upper=0.6):
        num_samples = 10
        r = 0.005
        ...
        max_violation = torch.maximum(
            lipschitz_ratios - local_lip_upper, torch.tensor(0.0))
    ...
    def configure_optimizers(self):
        ...
        #     optimizer_V = torch.optim.Adam(
        #         list(self.V.parameters()), lr=self.primal_learning_rate)
        #     optimizer_controller = torch.optim.Adam(list(self.V.parameters(
        #     )) + list(self.controller.nn.parameters()), lr=self.primal_learning_rate)
...
class TrainerRetrain(pl.LightningModule):
    ...
    def on_train_epoch_end(self):
        ...
        if (total.item() <= self.threshold):
            ...
            print("saved!")
            if self.epoch == 1:
                torch.save(self.V, "/Users/javiergutierrez/Documents/Thesis/Thesis_Repo/InvertedPendulum_UIUC/models_epsl_5e-3_lip3_4/final_cert_reached.pt")
                exit()
        else:
            self.init_val += 1
            self.log("saved_loss", torch.tensor(
...
def train_model(out_train_file, out_val_file, out_model_file, out_controller_file, threshold, initial_controller_file, out_timing_file, device="cpu", lr=1e-3):
    model = InvertedPendulum()
    V = LyapunovNetworkV(model).to(device)
    # V = torch.load("models_epsl_5e-3_lip3_2/cert_0.pt", weights_only=False)
    datamodule = SampleData(out_train_file, out_val_file, model, device=device)
    ...
    #     )])
    pltrainer = pl.Trainer(max_epochs=200, accelerator='cpu', callbacks=[
...
def retrain_model(epoch, counterexamples, counterexample_ranges, in_train_file, in_traj_file, out_train_file, out_val_file, out_traj_file, in_model_file, in_controller_file, out_model_file, out_controller_file, counterexample_folder, out_timing_file, threshold, lr=1e-4, device="cpu"):
    # counterexamples = read_in_points(in_data_file, in_folder)
    model = InvertedPendulum()
    V = torch.load(in_model_file, weights_only=False, map_location=device)
    controller = Controller(file_name=in_controller_file,
...
if __name__ == "__main__":
    ...
    train_model(5, 6, 0.5, cur_data_file, cur_val_file, cur_model_file,
                cur_controller_file, threshold, initial_controller_file)
