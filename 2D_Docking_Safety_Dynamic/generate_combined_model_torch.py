...
from training_exp_safe_mask import LyapunovNetworkV, TwoDimDocking, Dynamic
...
def combined_model(file_1,file_2,file_3, output_file): 
    ...
    input_data_one = torch.randn(10, 4, dtype=torch.float32)  
    input_data_two = torch.randn(10, 4, dtype=torch.float32)
    input_data_three = torch.randn(10, 6, dtype=torch.float32)

    input_data_one = torch.tensor([[1,1,1,1]], dtype=torch.float32)
    input_data_two = torch.tensor([[1,1,1,1]], dtype=torch.float32)
    input_data_three = torch.tensor([[1,1,1,1,1,1]], dtype=torch.float32)
    ...
    z = torch.randn(1,6, dtype=torch.float32,requires_grad=True)
