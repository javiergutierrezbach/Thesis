...
def simulate_traj(networkName, state, state_exact, docking_radius, max_dist, certificate, pert_rad, timeout):
    ...
    for i in range(TIMEOUT):
        ...
        if abs(state_exact[0]) < DOCK_RAD and abs(state_exact[1]) < DOCK_RAD:
            ...
            dockStatus = True
            break
        if abs(state_exact[0]) >= MAX_DIST or abs(state_exact[1]) >= MAX_DIST or math.sqrt(math.pow(state_exact[2],2) + math.pow(state_exact[3],2)) >= (gen_vel_limit(state_exact[0], state_exact[1]) - 0.0001):
...
if __name__ == "__main__":
    ...
    controller_true_name = 'controller_3'
    networkName_init = f"onnx_controllers/{controller_name_init}.onnx"
    networkTrueName = f"onnx_controllers/{controller_true_name}.onnx"
    ...
    for i in range(len(x0)):
        ...
        dockStatus, steps, safe = simulate_traj(networkTrueName, x0[i], x0[i], docking_radius, max_dist, cert, pert_rad, timeout_epochs)
