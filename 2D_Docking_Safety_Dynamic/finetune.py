...
def finetune_dynamic(dynamic, data_file, threshold, out_file):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-8, weight_decay=0.0)  # slightly lower lr for finetuning
    ...
    print(f"Original dynamic training data MSE: {threshold:.4}\n")


    while avg_loss > threshold and epoch < 30:
        epoch_loss = 0.0
...
if __name__ == "__main__":
    ...
    finetune_data = append_training_set("dynamic_data/train_0.pt", "counterexamples/counterexample_0.pt", 0.05,"dynamic_data/finetune_data.pt")
