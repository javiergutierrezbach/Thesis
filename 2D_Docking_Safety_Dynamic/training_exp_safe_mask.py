...
class Controller():
    def __init__(self, dynamic, file_name = "fixed_controller_20n.pt", isInitial=False, t=1):
        ...
        if (isInitial):
            self.nn = LearnedController().float()
            self.nn.load_state_dict(torch.load(self.file_name))
        else:
            self.nn = torch.load(self.file_name).float()
    ...
    def next_step(self, x):
        ...
        next_step = self.dynamic(total_input)
...
class Trainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        lip_term = self.lipschitz_loss()
        total_loss = descent_acc + descent_term + nongoal_term + nongoal_acc + goal_term + goal_acc + lip_term
...
class TrainerRetrain(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        lipterm = self.lipschitz_loss()
        total_loss = descent_acc + descent_term + nongoal_term + nongoal_acc + goal_term + goal_acc + lipterm
        opt_v, opt_f = self.optimizers()
...
def train_model(st_pos, unsafe_pos, vel_limit, out_train_file, out_val_file, out_model_file, out_controller_file, threshold, initial_controller_file, dynamic_file, lr=5e-3):
...
def retrain_model(epoch, counterexamples, counterexample_ranges, st_pos, unsafe_pos, vel_limit, in_train_file, out_train_file, out_val_file, in_model_file, in_controller_file, out_model_file, out_controller_file, dynamic_file, threshold, lr=1e-4):
    ...
    for param in dynamic.parameters():
        param.requires_grad = False
    controller = Controller(dynamic, file_name = in_controller_file, isInitial = False)
...
if __name__ == "__main__":
    ...
    train_model(4, 5, 0.22, cur_data_file, cur_val_file, cur_model_file, cur_controller_file, threshold, initial_controller_file, dynamic_file)
