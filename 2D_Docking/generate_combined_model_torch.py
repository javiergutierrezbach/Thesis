...
from training_exp_mask import LyapunovNetworkV, TwoDimDocking, Dynamic
...
def combined_model(file_1,file_2,file_3, output_file): 
    ...
    class CombinedNetwork(nn.Module):
        ...
        def forward(self, x, y, z):
            ...
            output_A = self.model_A.linear_relu_stack(input_A)
    ...
    input_data_one = torch.randn(10, 4, dtype=torch.float64)  
    input_data_two = torch.randn(10, 4, dtype=torch.float64)
    input_data_three = torch.randn(10, 6, dtype=torch.float64)

    input_data_one = torch.tensor([[1,1,1,1]], dtype=torch.float64)
    input_data_two = torch.tensor([[1,1,1,1]], dtype=torch.float64)
    input_data_three = torch.tensor([[1,1,1,1,1,1]], dtype=torch.float64)
    ...
    x = torch.randn(1,4, dtype=torch.float64,requires_grad=True)
    y = torch.randn(1,4, dtype=torch.float64,requires_grad=True)
